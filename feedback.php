...
// Define current year and academic year
$currentYear = date("Y");
...
if (isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO feedback (AccessibilityWarden, AccessibilityMember, RedressalProblem, Room, Mess, HostelSurroundings, OverallRating, FeedbackMessage, userId, academicYear) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $mysqli->prepare($query);
    $rc = $stmt->bind_param('ssssssssis', $acceswardent, $accesmember, $redproblem, $Room, $Mess, $hstelsor, $overall, $feedback, $aid, $academicYear);
    $stmt->execute();
                            ...
                            <div class="col-md-12">
                                <div class="panel panel-primary">
                                    <div class="panel-body">
                                        <?php
                                        $uid = $_SESSION['login'];
                                        $stmt = $mysqli->prepare("SELECT emailid FROM registration WHERE emailid=? || userPrn=? ");
                                        ...
                                        if ($rs) {
                                            $ret = $mysqli->prepare("SELECT id FROM feedback WHERE userId=? AND academicYear=?");
                                            $ret->bind_param('is', $aid, $academicYear);
                                            $ret->execute();
                                            $ret->bind_result($count);
                                            $ret->fetch();

                                            if ($count > 0) {
                                                $ret = "SELECT * FROM feedback WHERE userId=? AND academicYear=?";
                                                $stmt = $mysqli->prepare($ret);
                                                $stmt->bind_param('is', $aid, $academicYear);
                                                $stmt->execute();
                                                $res = $stmt->get_result();
                                                $cnt = 1;
                                                while ($row = $res->fetch_object()) {
                                                    ...
                                                    <table id="zctb" class="table table-bordered" cellspacing="0" width="100%" border="1">
                                                        <tr>
                                                            <td colspan="6" style="text-align:center; color:blue"><h4>Feedback Details for Academic Year <?php echo $row->academicYear; ?></h4></td>
                                                        </tr>
                                                        <tr>
                                                ...
                                                if (isFeedbackOpen()) {
                                                    ...
                                                    ?>
                                                    <div>The feedback form is available only in May and June each year.</div>
                                                    <?php
                                                }
                                            }
                                        } else {
