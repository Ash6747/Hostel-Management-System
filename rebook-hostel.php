...
if(isset($_POST['submit']) && isset($_POST['room'])){
	...
	if($count >= $seater){
		...
		$checkquery = "SELECT * FROM registration WHERE userPrn=? AND status='leave'";
		...
		// Close the statement
		$checkstmt->close();
		// echo isset($row->academicYear);
		if(isset($row->academicYear) && $row->academicYear == $academicYear){
			echo "<script>alert('You can not Apply same year try next Academic Year');</script>";
		}else{ 
// INSERT INTO `registration`(`id`, `roomno`, `seater`, `feespm`, `foodstatus`, `stayfrom`, `academicYear`, `course`, `class`, `userPrn`, `firstName`, `middleName`, `lastName`, `gender`, `contactno`, `emailid`, `egycontactno`, `guardianName`, `guardianRelation`, `guardianContactno`, `corresAddress`, `corresCIty`, `corresState`, `corresPincode`, `pmntAddress`, `pmntCity`, `pmnatetState`, `pmntPincode`, `postingDate`, `updationDate`, `status`, `comment`, `verifiedBy`, `clgName`, `hostelName`, `statusUpdationDate`) VALUES ('[value-1]','[value-2]','[value-3]','[value-4]','[value-5]','[value-6]','[value-7]','[value-8]','[value-9]','[value-10]','[value-11]','[value-12]','[value-13]','[value-14]','[value-15]','[value-16]','[value-17]','[value-18]','[value-19]','[value-20]','[value-21]','[value-22]','[value-23]','[value-24]','[value-25]','[value-26]','[value-27]','[value-28]','[value-29]','[value-30]','[value-31]','[value-32]','[value-33]','[value-34]','[value-35]','[value-36]')
			...
			$rc = $stmt->bind_param(
			'sidissssssssisississsisssisssss',
			$roomno,
			$seater,
			$feespm,
			$roomId,
			...
			$clgName,
			$hostelName,
			$userPrn // This is the identifier to find the specific record to update
			);
			if ($stmt->execute()) {
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title">Re-Apply </h2>
							...
							<div class="col-md-12">
								<div class="panel panel-primary">
									...
									<div class="panel-body">
										...
										// Close the statement
										$stmt->close();
										if($row->status == 'leave' || $row->status == 'rejected'){ 
									...
									?>			
										...
										{
										?>
										<option value="<?php echo $row->room_no.",".$row->gender.",".$row->seater.",".$row->yearlyFees.",".$row->hostelName.",".$row->clgName.",".$row->id;?>" > <?php echo $row->clgName." ".$row->room_no; ?> </option>
										<?php } ?>
										</select> 
										...
										<div class="col-sm-8">
										<input type="tel" name="econtact" id="econtact" pattern="[0-9]{10}" minlength="10" maxlength="10" class="form-control" required="required">
										</div>
										</div>
										...
										<div class="col-sm-8">
										<input type="tel" name="gcontact" id="gcontact" pattern="[0-9]{10}" minlength="10" maxlength="10" class="form-control" required="required">
										</div>
										</div>	
