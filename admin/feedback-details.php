		...
		<div class="content-wrapper">
			...
			<table id="zctb" class="table table-bordered " cellspacing="0" width="100%" border="1">
						 ...
						 <span style="float:left" ><i class="fa fa-print fa-2x" aria-hidden="true" OnClick="CallPrint(this.value)" style="cursor:pointer" title="Print the Report"></i></span>			
									<tbody>
<?php	
$aid=intval(decrypt($_GET['userPrn']));
	$ret="SELECT 
				r.*,
				COUNT(rep.userPrn) AS reportCount,
				SUM(rep.roomFeesphy) AS hostelFee,
				SUM(rep.reportTotalAmount) AS totalfee
			FROM 
				registration r
			LEFT JOIN 
				report rep
			ON 
				r.userPrn = rep.userPrn
			WHERE r.userPrn = ?
			GROUP BY 
			r.userPrn;";
...
<tr>
<td colspan="6" style="text-align:center; color:blue"><h4>Room Realted Info</h4></td>
</tr>
<tr>
...
<td><b>Duration:</b></td>
<td><?php echo $row->reportCount;?> Months</td>
</tr>


<tr>
<th>Total Fee :</th>
<th colspan="5"><?php echo $row->totalfee;?></th>
</tr>
<tr>
