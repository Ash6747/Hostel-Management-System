...
// $seatsAvaibility = $_POST['seatsAvaibility'];
$totalSeat = $_POST['totalSeat'];
	if($seater < $occupiedSeats){
		echo"<script>alert('Room seats can not be less than occupied seats');</script>";
	}else{
		$seatsAvaibility = $seater-$occupiedSeats;
		$query="update rooms set seater=?,yearlyFees=?,seatsAvaibility=? where id=?";
		$stmt = $mysqli->prepare($query);
		$rc=$stmt->bind_param('iiii',$seater,$fees,$seatsAvaibility,$id);
		if($stmt->execute())
								...
								<div class="panel panel-default">
									...
									<div class="panel-body">
										<form method="post" class="form-horizontal">
											...
											<input type="number" name="totalSeat" value="<?php echo $row->seater;?>" hidden>
											<input type="number" name="occupiedSeats" value="<?php echo $row->occupiedSeats;?>" hidden>
											<input type="number" name="seatsAvaibility" value="<?php echo $row->seatsAvaibility;?>" hidden>
											...
											<div class="col-sm-8">
											<input type="text" class="form-control" name="fees" value="<?php echo $row->yearlyFees;?>" >
											</div>
											</div>
